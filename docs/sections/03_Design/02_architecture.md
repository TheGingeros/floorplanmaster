...
Technická analýza (kapitola 2.6) identifikovala klíčové technologické možnosti pro realizaci addonu: Geometry Nodes jako výpočetní jádro pro generování 3D geometrie, strukturální graf a NRG pro datovou reprezentaci půdorysu, modální operátory pro interaktivní kreslení a pojmenované atributy jako synchronizační most mezi Pythonem a Blenderem. Tato sekce překládá analytické nálezy do konkrétní softwarové architektury — z jakých bloků se systém skládá (statika), jak tyto bloky komunikují (dynamika) a jakými pravidly se tato komunikace řídí.
...
Blender v základu zná geometrické primitivy: vrchol, hranu, plochu. Nezná stěnu, junction ani místnost. Bez sémantické vrstvy by byl každý objekt v scéně pouze seskupením polygonů bez doménového kontextu — uživatel by musel ručně hlídat tloušťky, výšky, topologické sousednosti a konzistenci otvorů. Tato sémantická mezera je klíčovým zdůvodněním celé architektury. Nástroje addonu (Pencil Tool, manipulátory, panely) nejsou reimplementací obecných Blender operací jako `posun`, `vytažení` nebo `smazání`, ale doménově specifickými operacemi pracujícími výhradně na Vrstvě 1 a Vrstvě 2. Posun junctionu nespouští obecný Blender transform — přepočítá quady přilehlých stěn, ověří inset podmínky otvorů a aktualizuje všechny dotčené místnosti ve Vrstvě 2. Tato distinktce platí pro každou operaci v addonu: Controller vždy volá grafové metody Modelu, nikdy nepíše přímo do Blender geometrie. Analogický přístup volí Revit, ArchiCAD nebo FreeCAD Arch, které jsou také postaveny na obecném geometrickém jádru, ale přidávají nad ním sémantickou vrstvu entit (stěna, místnost, podlaží) s vlastními omezeními a vztahy.
...
Třívrstvá hybridní architektura je navržena tak, aby budoucí rozšíření nevyžadovala zásah do jádra Vrstvy 1 ani 2:
