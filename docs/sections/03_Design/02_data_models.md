...
Zatímco předchozí kapitola definovala procesy a tok dat (architekturu MVC), tato sekce představuje exaktní „slovník“ celého addonu. Definuje fyzické struktury, pravidla a hranice, ve kterých se data mohou pohybovat. Slouží jako pevná sada pravidel mezi matematickým jádrem v Pythonu a 3D prostředím Blenderu.
...
Následující specifikace neslouží jen jako přehled proměnných. Detailně definuje povolené datové typy, API metody, a především validační omezení, která garantují, že uživatel nemůže vytvořit architektonicky neplatný stav.
