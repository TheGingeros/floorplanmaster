...
**Problém dvojího kliknutí a jeho řešení.** Dvoustupňový přístup naivně zavádí dvojí kliknutí: uživatel nejdřív klikne na objekt (aktivace), pak klikne na stěnu (výběr). Řešení je **smart select**: LMB handler nejdřív testuje, zda klik zasáhl mesh FloorPlan objektu. Pokud zasáhl mesh neaktivního FloorPlan objektu, handler tento objekt nejdřív aktivuje (standardní Blender `ops.object.select_all` + set active) bez výběru stěny — uživatel jedním klikem přepne kontext. Pokud zasáhl mesh již aktivního FloorPlan objektu, handler pokračuje na raycast stěny a vybere ji. Toto chování přesně odpovídá tomu, jak Blender Object Mode funguje pro obecné objekty: první klik aktivuje, druhý klik (na stejném objektu) by mohl otevřít detail.
