...
Vývoj tužkového nástroje byl doprovázen sérií propojených problémů, z nichž většina se projevila teprve při testování s reálnou dispozicí obsahující přibližně čtyřicet stěn. Prvním a nejzávažnějším byl výkonnostní problém (Issue #28): každý potvrzený vrchol spouštěl plnou synchronizaci Vrstvy 3 --- přepočet stěnových obrysů, zápis osmi named atributů, reevaluaci GN modifikátoru a serializaci grafů do JSON custom property. S rostoucím počtem stěn cena každého kliknutí rostla lineárně, přičemž celková cena za nakreslení sekvence $W$ stěn dosahovala $O(W^2)$. Řešením bylo přesunutí plného syncu na konec celé kreslicí sekvence a náhrada per-klik vizualizace čistě Pythonovými výpočty stěnových obrysů bez volání `bpy`. Cena per-klik klesla na $O(W)$; GN evaluace proběhne jednou při ukončení nástroje.

Druhým problémem bylo spuštění více instancí modálního operátoru (Issue #1): stisknutí klávesy D při aktivním tužkovém nástroji spustilo novou instanci, která zaregistrovala duplicitní GPU handlery a začala přetahovat události z první instance. Oprava vyžadovala guard na začátku `invoke()`: pokud je `_pencil_state is not None`, operátor okamžitě vrátí `{'CANCELLED'}`.

Třetím problémem byla obnova pohledu po deaktivaci (Issues #19, #30). Prvotní implementace obnovovala uloženou matici pohledu bezpodmínečně --- pokud uživatel v průběhu kresby ručně otočil viewport a ukončil nástroj z perspektivy, pohled se nechtěně přeskočil zpět na uloženou horní ortografii. Oprava podmínila obnovu pohledu: matice se obnoví pouze tehdy, pokud viewport stále zobrazuje horní ortografii, jinak se zachová aktuální pohled uživatele.
...
Implementace metadat místností odhalila specifický problém persistence jmen přes Undo a reload. Jméno místnosti žije v Python objektu v paměti (`Room.name`), který Undo/Redo neobnovuje --- Blender při Undo obnoví mesh, ale Python grafy v `_graph_store` zůstanou ve stavu po poslední operaci. Výsledkem je, že přejmenování místnosti provedené před Undo krokem může po Undo zůstat zachováno, i když topologie se vrátila o krok zpět. Řešením je psaní jmen místností do JSON custom property při každém syncu a jejich rekonstrukce z JSON při každém `reset_graphs_for_obj()`. Tím je mesh vždy autoritativní zdroj pravdy a Python objekty jsou pouze odvozená cache rekonstruovatelná z meshe.
