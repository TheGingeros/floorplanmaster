...
Blender @blender je primárně 3D polygonální modelovací nástroj. Nabízí obrovskou flexibilitu, pro specifické potřeby architektonického navrhování však postrádá nativní nástroje. Tvorba půdorysů a 3D dispozic budov je v čistém Blenderu zdlouhavá a destruktivní --- každá změna vyžaduje manuální opravu okolní geometrie, posouvání vrcholů a přepočítávání otvorů.

Cílem projektu je tedy vytvoření rozšiřovacího modulu zaměřeného na parametrické modelování prostorových dispozic. Uživatel bude moci prostorové dispozice intuitivně vytvářet a nedestruktivně upravovat pomocí parametrů; primárním cílem je urychlit ranou fázi architektonického návrhu.
...
Izolace kódu závislého na Blenderu od čistého Pythonu nevznikla z uměle v rámci návrhu, ale z bezprostřední praktické nutnosti. Jakmile byly napsány první testy pro strukturální graf, pytest okamžitě selhal: `import bpy` ve vrcholku modulu způsobil `ModuleNotFoundError`, protože mimo Blender Python interpret toto #gls("api", long: false) neexistuje. Vzor s podmíněným importem byl proto zaveden hned na začátku jako obranný mechanismus, nikoliv jako pozdní refaktor. Přímým důsledkem je, že veškerá základní logika --- topologické grafy, validátory, matematické utility --- je dostupná jako standardní Python balíček a pokrytá automatickými testy bez jakékoliv závislosti na Blenderu. Do budoucna je možné toto oddělení udělat více elegenantně a návrh i implementace s tím počítají. 
...
Nejpodstatnější algoritmický problém Vrstvy 1 se projevil až při testování T-spojů a X-spojů. Prvotní implementace funkce pro výpočet rohů stěnového quadu procházela všechny sousední stěny v daném vrcholu a vybírala průsečík postranní přímky stěny s nejmenším absolutním parametrem $|t|$ podél přímky --- kde $t$ je skalární parametr parametrické rovnice přímky $P(t) = A + t dot.op arrow(d)$ udávající vzdálenost průsečíku od referenčního bodu $A$ ve směru $arrow(d)$. Pro L-spoje (dva sousedé ve vrcholu) tato strategie fungovala korektně. Jakmile se ale vrchol spojoval se třemi nebo více stěnami, `|t|` minimum vybíralo špatnou stranu --- rohový bod quadu přeskočil přes sousední stěnu a vytvořil překrývající se geometrii. Oprava vyžadovala přepracování výpočtu na angular-sort algoritmus: `_junction_entries()` seřadí všechny stěny v daném vrcholu podle úhlu jejich odchozího směru, čímž vznikne cyklus sousedů. Levý roh stěny se poté hledá jako průsečík levé postranní přímky stěny $i$ s pravou postranní přímkou angulárně-sousední stěny $i+1$ v CCW pořadí. Tento přístup je deterministický pro libovolný počet stěn ve vrcholu.
...
#gls("gn", long: false) modifikátor přebírá od Vrstvy 3 připravený base mesh a autonomně generuje finální trojrozměrnou geometrii. #gls("gn", long: false) strom je sestaven při první aktivaci addonu a nese interní číslo verze; při nesouladu verze se strom automaticky přebuduje. Tok zpracování ve stromu probíhá ve třech krocích: oddělení stěnových faces (`is_wall == 1`), oddělení opening cutter faces (`is_opening == 1`) a vytažení stěn podle per-face `wall_height`.

Kritický problém #gls("gn", long: false) fáze přišel s implementací dveřních otvorů. EXACT Boolean solver (#gls("gn", long: false) uzel Mesh Boolean DIFFERENCE/EXACT) vyžaduje watertight cutter mesh, jehož geometrie nesmí zasahovat mimo těleso, do kterého řeže. Dveřní cutter začínal na `z = 0` --- přesně na spodní ploše stěny. Numerická tolerance EXACT solveru tento stav vyhodnotila jako průsečík s podlahou stěny, což vedlo k nestabilnímu chování: díra buď nevznikla vůbec, nebo solver zobrazil cutter jako solidní těleso. Okenní otvory přitom fungovaly bezchybně, protože jejich cutter začíná na výšce nadpraží (`z = sill_height - ε > 0`) --- celý cutter leží uvnitř stěnového solidu. Oprava pro dveře spočívala v posunutí dolní hrany cutteru na `z = +0,01 m` (konstanta `OPENING_CUTTER_Z_OVERSHOOT`), čímž se vyloučila numerická interakce s podlahou. Výsledná mezera 1 cm u podlahy dveří je v praxi nepozorovatelná a pro účely addonu zanedbatelná. Do budoucna by bylo ideální problém vyřešit exaktním výřezem a opravou geometrie, aby při práci nevznikaly nežádoucí artefakty.
...
Vývoj tužkového nástroje byl doprovázen sérií propojených problémů, z nichž většina se projevila teprve při testování s reálnou dispozicí obsahující přibližně čtyřicet stěn. Prvním a nejzávažnějším byl výkonnostní problém: každý potvrzený vrchol spouštěl plnou synchronizaci Vrstvy 3 --- přepočet stěnových obrysů, zápis osmi named atributů, reevaluaci #gls("gn", long: false) modifikátoru a serializaci grafů do #gls("json", long: false) custom property. S rostoucím počtem stěn cena každého kliknutí rostla lineárně, přičemž celková cena za nakreslení sekvence $W$ stěn dosahovala $O(W^2)$. Řešením bylo přesunutí plného syncu na konec celé kreslicí sekvence a náhrada per-klik vizualizace čistě Pythonovými výpočty stěnových obrysů bez volání `bpy`. Cena per-klik klesla na $O(W)$; #gls("gn", long: false) evaluace proběhne jednou při ukončení nástroje.
...
Finalizační operátor překonává architektonické oddělení procedurálního a statického modelu. Parametrický FloorPlan objekt nese celou historii topologie v #gls("json", long: false) a vizualizaci deleguje na #gls("gn", long: false) modifikátor; statický mesh je naopak prostá geometrická data bez sémantiky. Před samotným bake operátor rekonstruuje grafy z aktuálního stavu objektu, vygeneruje finální geometrii přes `final_mesh_builder.py` s geometricky přesnými spoji a odstraní #gls("gn", long: false) modifikátor i #gls("json", long: false) custom property. Ochrana vstupu do Edit Mode zabraňuje situaci, kdy by uživatel neúmyslně přepsal #gls("gn", long: false) mesh ručními editacemi: handler zachytávající přechod do mesh editace nabídne tři volby --- zrušit přechod, provést bake a pak editovat, nebo vstoupit a ztratit sémantická data. Tato ochrana brání tichým chybám, které by jinak vznikly použitím standardní Blender editace na procedurálním objektu.
