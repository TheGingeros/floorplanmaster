...
Geometry Nodes (GN) zastupují deklarativní, paralelní paradigma: uživatel definuje systém pravidel aplikovaných na celou geometrii současně. Data jsou reprezentována jako pole atributů vázaných na různé domény (vrcholy, hrany, plochy, instance), přičemž výpočet probíhá v nativním kódu C++ s plným multithreadingem.
...
Mesh geometrie zachycuje tvary stěn, ale neví nic o tom, že dva prostory spolu sousedí, že jeden z nich je kuchyň nebo že dveřní otvor mezi nimi má šíři 900~mm. Pro addon, který má automaticky detekovat místnosti, zobrazovat jejich plochy a reagovat na každou topologickou změnu, je potřeba datová vrstva nad geometrií. Tato sekce popisuje hybridní model dvou grafů: strukturálního skeletu stěn a sémantického grafu místností, jejich vzájemné propojení a strategii detekce místností, která zajišťuje konzistenci databáze při libovolné editaci půdorysu.

Pro efektivní reprezentaci struktury sítě místností je pro parametrický addon nedostatečná samotná mesh geometrie --- systém musí vědět, že prostor A je obývací pokoj a sousedí s prostorem B, který je kuchyní. Zvolený hybridní model využívá knihovnu NetworkX a pracuje se dvěma vzájemně propojenými vrstvami.

==== Vrstva 1: Topologický skelet (strukturální graf)

Vrstva 1 spravuje čistá grafová data o uzlových bodech (junctions) a stěnách. Uzly ($V_s$) jsou body spojení stěn s XY souřadnicemi, hrany ($E_s$) jsou osy stěn. Graf je planární --- každá hrana v něm reprezentuje fyzickou stěnu, která odděluje buď dva prostory, nebo prostor od exteriéru. NetworkX zde počítá planární embedding a identifikuje uzavřené cykly (Face), tedy místnosti, pomocí algoritmů pro hledání cyklů v planárním grafu. Tato vrstva slouží jako vodicí geometrie pro Geometry Nodes.

==== Vrstva 2: Sémantický dual (Node Relation Graph)

Vrstva 1 uchovává topologii --- jde o čistě geometrickou informaci. Pro architektonické dotazy (jaký typ má místnost, sousedí tyto dvě místnosti, jaká je celková plocha ložnic) nestačí. Vrstva 2 tento sémantický rozměr doplňuje jako Node Relation Graph (NRG) --- duální reprezentace, kde uzly ($V_r$) reprezentují místnosti a nesou nefyzikální metadata (název, typ, plocha, obvod, centroid), zatímco hrany ($E_r$) reprezentují vztah sousednosti dvou místností sdílejících fyzickou stěnu. Hrany mohou být typizovány: `WALL`, `DOOR`, `WINDOW` nebo `PASSAGE`.

Alternativou k NRG je prostý seznam místností bez explicitně modelovaných vztahů sousedství. Dotaz „sousedí místnost A s místností B?" by v takovém případě vyžadoval průchod Vrstvou 1 s časovou složitostí $O(E)$ na každý dotaz. NRG tento dotaz redukuje na $O(1)$ vyhledání v adjačenčním seznamu. Implementace přes NetworkX navíc umožňuje využít existující grafové algoritmy (BFS pro dostupnost, Dijkstra pro nejkratší trasu) bez vlastní implementace. Vrstva 2 je vždy přísně odvozená z Vrstvy 1 --- nikdy není editována přímo (vyjma uživatelských metadat: název, typ).
