...
Blender je modulární systém postavený na unikátním způsobu správy dat --- dualitě systému DNA a RNA. DNA (Blender's Data Architecture) definuje struktury C pro veškerá interní data scény, zatímco RNA (Runtime Notification Architecture) poskytuje reflexivní API, přes které Python přistupuje k těmto strukturám a reaguje na jejich změny @blender_dev.
